...
void SendClient(int userio,CSMsg * test);    //该函数是逻辑层在发送数据给客户端时，必须调用该接口才能发送数据出去。

void SendServer(int userio,SSMsg * test);	//该函数是发给另一个服务器（进程)

void TimeSend(bool kaiguan,int index,int miao,int userid); //该函数是逻辑层只要用到定时器就会调用，给上层一个定时接口
