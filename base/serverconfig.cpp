...
bool ServerConfig::Init(std::string configname)
{
	...
	//读取服务器的IP地址和端口号
	{
		TiXmlElement *server_element = RootElement->FirstChildElement("server");
		...
		if (iRet)
		{
			std::cout<<"InitServerConfigg failed : "<<std::cout<<iRet<<std::endl;;
...
int ServerConfig::InitServerConfigg(TiXmlElement *RootElement)
{
		//开始读取服务器xml文件节点的数据
	...
	{
		if (!GetSubNodeValue(dataElement, "ip", server.ip) || server.ip.empty())
		{
			return -1;
		}
		
		if (!GetSubNodeValue(dataElement, "port", server.port) || server.port < 0 )
		{
			return -2;
		}
